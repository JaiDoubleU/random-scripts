...
// source: https://raw.githubusercontent.com/thisisevanfox/nhl-my-team-ios-widget/main/NHL-MyTeam-Widget.js; docs: https://github.com/thisisevanfox/nhl-my-team-ios-widget; hash: 1030627177;
...
// false (x - x - x)
const SHOW_STATS_DESCRIPTION = false;
...
const LOGO_SIZE = new Size(65, 65);
const TOP_ROW_SIZE = new Size(308, 18);
const SCORE_FONT = Font.regularSystemFont(35);
const GAME_DAY_DATE_FORMAT = { weekday: 'long', month: 'short', day: 'numeric' };
const GAME_DAY_DATE_FORMAT_SM = { weekday: 'short', month: 'short', day: 'numeric' };

const darkBackground = new Color("111111");
const lightBackground = new Color("#D6D6D6");
...
const WIDGET_BACKGROUND = DARK_MODE ? new Color("#111111") : new Color("#D6D6D6");
...
async function createMediumWidget() {
  ...
  //oWidget.setPadding(0, DEFAULT_PADDING, 0, DEFAULT_PADDING);
  if (WIDGET_URL.length > 0) {
    oWidget.url = WIDGET_URL;
  }
...
async function addMediumWidgetData(oWidget) {
  ...
  oTopRow.cornerRadius = 3;
  oTopRow.size = TOP_ROW_SIZE;
  oTopRow.setPadding(DEFAULT_PADDING, 0, DEFAULT_PADDING, DEFAULT_PADDING);
  ...
  if (oGameData != null) {
    ...
    if (
      ...
      const sDatePart = bGameIsToday ? "Today" :
        dGameDate.toLocaleString([], GAME_DAY_DATE_FORMAT_SM);
//          {
//          year: "numeric",
//          month: "2-digit",
//          day: "2-digit"
//        });
      ...
      //console.error(oGameData);
      const sGameDateText = `${sDatePart}, ${sTimePart}`;
      const sVenueText = oGameData.venue != "" ? ` @ ${oGameData.venue}` : ``;
      oHeadingText = oHeadingStack.addText(`${sGameDateText}`);
    }
    oHeadingText.font = Font.regularSystemFont(14);
    ...
    oFutureGamesStack.cornerRadius = 3;
    ...
    oHeadingText.font = Font.regularSystemFont(11);
    oHeadingText.textColor = getColorForCurrentAppearance();
...
async function addSmallWidgetData(oWidget) {
  ...
  if (oGameData != null) {
    ...
    const GAME_DATE_FONT = Font.regularSystemFont(9);
    
    const dGameDate = new Date(oGameData.gameDate);
     dGameDate.toLocaleString([], GAME_DAY_DATE_FORMAT_SM);
    const dLocalDate = dGameDate.toLocaleString([], GAME_DAY_DATE_FORMAT_SM);
...
//    });
    ...
    oGameDateText.font = GAME_DATE_FONT;
    ...
    if (SHOW_STATS_AND_STANDINGS) {
      ...
      oDivider.font = Font.regularSystemFont(6);
...
async function addHomeTeamStack(oNextGameStack, oGameData) {
  ...
  oHomeTeamStack.setPadding(5, 5, 5, 5);
  await setStackBackground(oHomeTeamStack);
  oHomeTeamStack.cornerRadius = 12;
  oHomeTeamStack.size = new Size(150, 0);
  
  //oHomeTeamStack.addSpacer(5);
  
  const homeText = oHomeTeamStack.addText('        Home ');
  homeText.font = Font.systemFont(10);
  homeText.centerAlignText();
  oHomeTeamStack.addSpacer(5);
  ...
  if (SHOW_LIVE_SCORES) {
    ...
    oHomeTeamGoalsText.font = SCORE_FONT;
...
async function addAwayTeamStack(oNextGameStack, oGameData) {
  ...
  oAwayTeamStack.setPadding(5, 5, 5, 5);
  await setStackBackground(oAwayTeamStack);
  oAwayTeamStack.cornerRadius = 12;
  oAwayTeamStack.size = new Size(150, 0);
  
  const awayText = oAwayTeamStack.addText('        Away');
  awayText.font = Font.systemFont(10);
  awayText.centerAlignText();
  
  oAwayTeamStack.addSpacer(5);
  ...
  if (SHOW_LIVE_SCORES) {
    ...
    oAwayTeamGoalsText.font = SCORE_FONT;
...
async function setStackBackground(oStack) {
  if (
    ...
    console.error('no background active');
